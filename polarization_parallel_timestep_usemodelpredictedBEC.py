...
Note that "xlo, ylo, zlo = [0, 0, 0]" is assumed, which is the case for our simulations. should be made more general.
'''
...
def calculate_polarizationperunitcell(unitcell: pd.DataFrame) -> list[float]:
    ...
    latt_x = np.mean(hq.nlargest(4, unitcell[ (unitcell['elements']=='Ba') | (unitcell['elements']=='Sr') ]['x'])) - np.mean(hq.nsmallest(4, unitcell[ (unitcell['elements']=='Ba') | (unitcell['elements']=='Sr') ]['x']))
    latt_y = np.mean(hq.nlargest(4, unitcell[ (unitcell['elements']=='Ba') | (unitcell['elements']=='Sr') ]['y'])) - np.mean(hq.nsmallest(4, unitcell[ (unitcell['elements']=='Ba') | (unitcell['elements']=='Sr') ]['y']))
    latt_z = np.mean(hq.nlargest(4, unitcell[ (unitcell['elements']=='Ba') | (unitcell['elements']=='Sr') ]['z'])) - np.mean(hq.nsmallest(4, unitcell[ (unitcell['elements']=='Ba') | (unitcell['elements']=='Sr') ]['z']))
    ...
    return [com_x, com_y, com_z, Px, Py, Pz, Px_bec, Py_bec, Pz_bec, latt_x, latt_y, latt_z]
...
def loopthroughTi(xhi:float, yhi:float, zhi:float, data: pd.DataFrame) -> pd.DataFrame:
    ...
    centraluc = pd.DataFrame({}, columns=['x', 'y', 'z', 'px', 'py', 'pz', 'px_bec', 'py_bec', 'pz_bec', 'latt_x', 'latt_y', 'latt_z'])
    ...
    boundaryuc = pd.DataFrame({}, columns=['x', 'y', 'z', 'px', 'py', 'pz', 'px_bec', 'py_bec', 'pz_bec', 'latt_x', 'latt_y', 'latt_z'])
...
def find_surroundingTi(xhi:float, yhi:float, zhi:float, data: pd.DataFrame, index: int, tag: str) -> pd.DataFrame:
    ...
    if tag == 'none':
        ...
        mask_A = (data['elements'] == 'Ba') | (data['elements'] == 'Sr')
        ...
        surrounding_A = data[mask_A & mask_x & mask_y & mask_z]
        surrounding_O  = data[mask_O  & mask_x & mask_y & mask_z]
        # print(surrounding_Ba)
        # print(surrounding_O)
        if len(surrounding_A) != 8:
            print('wrong')
        ...
        surrounding_A  = data[mask_A  & (mask_x | mask_x_1 | mask_x_2) & (mask_y | mask_y_1 | mask_y_2) & (mask_z | mask_z_1 | mask_z_2)]
        ...
        if len(surrounding_O) != 6:
            print('wrong')
    return pd.concat([surrounding_A, surrounding_O, surrounded_Ti])
...
def process_timestep(i):
    ...
    conditions = [(data['elements'] == 'Ba'),    (data['elements'] == 'O'),    (data['elements'] == 'Ti'), (data['elements'] == 'Sr')]
    charges = [2, -2, 4, 2]  # USER: nominal charges are used for BaTiO3 (Ba: +2, O: -2, Ti: +4)
    data['charge'] = np.select(conditions, charges, default=0)
    weights = [1/8, 1/2, 1, 1/8] # USER: weights of atoms in a unit cell
...
# def main(): # only for profiling
    ...
    interval = 50       # depends on the output frequency of lammps
